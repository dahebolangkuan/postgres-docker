<#
.SYNOPSIS
    Verifies that all PostgreSQL extensions in the Dockerfile.lite3 image are installable and functional.
.DESCRIPTION
    This script starts a temporary container from the 'postgres-lite3' image,
    waits for it to become ready, and then iteratively attempts to create and test
    each extension.
#>

param(
    [string]$ImageName = "postgres-lite-opt-i18n"
)

$ContainerName = "postgres-lite3-test-$(Get-Random)"
$PostgresPassword = "password"

Write-Host "Starting validation for image: $ImageName" -ForegroundColor Cyan

# 1. Start the container
# We explicitly check if JIT is off effectively by the container starting successfully.
...
try {
    ...
    if (-not $isReady) {
        ...
        Write-Error "PostgreSQL did not become ready. Logs: $logs"
    ...
    $extensions = @(
        ...
        @{ Name = "pg_cron";      Query = "SELECT count(*) FROM cron.job;" }
        @{ Name = "jsquery";      Query = 'SELECT ''{\"a\": 1}''::jsonb @@ ''a = 1''::jsquery;' }
        @{ Name = "pg_partman";   Query = 'SELECT schema_name FROM information_schema.schemata WHERE schema_name = ''partman'';' }
